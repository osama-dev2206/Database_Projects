...
-- ─────────────────────────────────────────────────────────────
-- CLEANUP  (delete in reverse FK order so re-running is safe)
-- ─────────────────────────────────────────────────────────────
...
DELETE FROM Members;
DELETE FROM BeltRanks;
DELETE FROM Persons;

-- Reset identity counters so IDs always start from 1
DBCC CHECKIDENT ('BeltTests',        RESEED, 0);
DBCC CHECKIDENT ('Subscriptions',    RESEED, 0);
DBCC CHECKIDENT ('MemberInstructors',RESEED, 0);
DBCC CHECKIDENT ('Payments',         RESEED, 0);
DBCC CHECKIDENT ('Qualifications',   RESEED, 0);
...
DBCC CHECKIDENT ('BeltRanks',        RESEED, 0);
...
-- ─────────────────────────────────────────────────────────────
-- 10. SUBSCRIPTIONS
--     Columns: SupscriptionId (identity), StartDate, EndDate,
--              Fees, MemberShipStatus, SMemberId (FK->Members),
--              SPaymentId (FK->Payments)
--     MemberShipStatus: 1 = Active, 0 = Inactive
-- ─────────────────────────────────────────────────────────────
INSERT INTO Subscriptions (StartDate, EndDate, Fees, MemberShipStatus, SMemberId, SPaymentId) VALUES
('2025-01-01', '2025-06-30', 500, 1, 1, 1),  -- SupscriptionId=1  Layla    Active   6-month  (Paid)
('2025-01-05', '2025-07-04', 500, 1, 2, 2),  -- SupscriptionId=2  Youssef  Active   6-month  (Paid)
('2025-01-10', '2025-07-09', 500, 1, 3, 3),  -- SupscriptionId=3  Nour     Active   6-month  (Paid)
('2025-02-01', '2025-07-31', 500, 1, 4, 4),  -- SupscriptionId=4  Karim    Active   6-month  (Paid)
('2025-02-05', '2025-08-04', 500, 1, 5, 5),  -- SupscriptionId=5  Dina     Active   6-month  (Paid)
('2025-02-10', NULL,         500, 0, 6, 6),  -- SupscriptionId=6  Tarek    Inactive EndDate=NULL (Payment Pending)
('2025-03-01', '2025-08-31', 500, 1, 7, 7);  -- SupscriptionId=7  Mariam   Active   6-month  (Paid)
